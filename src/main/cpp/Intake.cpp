...
Intake::Intake(DriverStation *ds, OperatorInputs *inputs, Lifter *lifter)
{
	...
	if (PCM_INTAKE_SOLENOIDVAC3 != -1)
		m_solenoidvac1 = new Solenoid(PCM_INTAKE_MODULE, PCM_INTAKE_SOLENOIDVAC3);
	if (PCM_INTAKE_SOLENOIDVAC4 != -1)
		m_solenoidvac2 = new Solenoid(PCM_INTAKE_MODULE, PCM_INTAKE_SOLENOIDVAC4);
    if (SPARK_VAC != -1)
...
void Intake::HatchLoop()
{
    ...
    case kHatchCapture:
        ...
        if (m_inputs->xBoxYButton(OperatorInputs::ToggleChoice::kToggle, 1 * INP_DUAL) && (m_waittime > PCM_PNEUMATIC_WAIT))
        {
            m_waittime -= .05;
        }
        break;
...
void Intake::CargoLoop()
{
    ...
    case kCargoEject:
        if (m_timer.Get() > 2)    // Magic # for eject
        {
            m_sparkcargo->Set(0);
            m_solenoidarm1->Set(false);
            m_solenoidarm2->Set(false);
